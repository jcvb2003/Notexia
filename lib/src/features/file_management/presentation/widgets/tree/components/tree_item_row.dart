...
class _TreeItemRowState extends State<TreeItemRow> {
  ...
  void _showMenu(BuildContext context, FileExplorerCubit cubit) {
    ExplorerContextMenu.show(
      ...
      onShowInFolder: () => cubit.showInFolder(widget.item.path),
      onSearchInFolder: () => cubit.searchInFolder(widget.item.path),
      onApplyAppearance: (icon, color) => cubit.updateItemMetadata(
        widget.item.path,
        icon: icon,
  ...
  IconData _getIcon() {
    if (widget.item.customIcon != null) {
      return FileIconUtils.resolveIcon(widget.item.customIcon);
    }
