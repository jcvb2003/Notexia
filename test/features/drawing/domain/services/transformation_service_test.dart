...
void main() {
  ...
  group('TransformationService.updateLineOrArrowEndpoint', () {
    test(
      'line com snapAngle true: vetor final � ajustado ao passo (pi/12)',
      () {
        final now = DateTime.now();
        ...
        final t = TransformationService();
        const worldPoint = Offset(80, 80); // for�a ~45�
        ...
        // vetor normalizado deve estar pr�ximo de 45� (dx � dy)
        expect((p1.dx - p0.dx).abs(), closeTo((p1.dy - p0.dy).abs(), 0.75));
      },
    );
    test('line com passo customizado (pi/6) ajusta �ngulo para ~30�', () {
      final now = DateTime.now();
      final line = LineElement(
        id: 'l2',
        x: 0,
