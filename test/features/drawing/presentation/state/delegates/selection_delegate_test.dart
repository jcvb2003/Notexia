  ...
  group('SelectionDelegate', () {
    group('setSelectionBox', () {
      test('sets selection box rect', () {
        ...
        expect(result.transform.selectionBox, rect);
      });

      test('clears selection box when null', () {
        const rect = Rect.fromLTWH(10, 10, 100, 100);
        final withBox = delegate.setSelectionBox(baseState, rect);
        final cleared = delegate.setSelectionBox(withBox, null);
    ...
    group('selectElementAt', () {
      test('selects element when point is inside bounds', () {
        ...
        expect(result.selectedElementIds, {'rect1'});
      });
      ...
      test('clears selection when point is outside all elements', () {
        ...
        expect(result.selectedElementIds, isEmpty);
      });
      ...
      test('selects topmost element when elements overlap', () {
        ...
        expect(result.selectedElementIds, {'overlap'});
      });

      test('multi-select toggles element in selection', () {
        // First select rect1
        final withRect1 = delegate.selectElementAt(
          baseState,
          const Offset(125, 125),
        );
        ...
        // Multi-select rect2
        final withBoth = delegate.selectElementAt(
          withRect1,
        ...
        final withOnlyRect2 = delegate.selectElementAt(
          withBoth,
          const Offset(125, 125),
          isMultiSelect: true,
        );
      ...
      test('does not re-select already selected element', () {
        final selected = delegate.selectElementAt(
          baseState,
          const Offset(125, 125),
        );
        final reselected = delegate.selectElementAt(
          selected,
          const Offset(125, 125),
        );
    ...
    group('selectElementsInRect', () {
      ...
      test('selects multiple elements in large rectangle', () {
        ...
        final result = delegate.selectElementsInRect(baseState, selectionRect);
        expect(result.selectedElementIds, {'rect1', 'rect2'});
      });
    ...
    group('setHoveredElement', () {
      test('sets hovered element id', () {
        ...
        expect(result.hoveredElementId, 'rect1');
      });

      test('clears hovered element id', () {
        final hovered = delegate.setHoveredElement(baseState, 'rect1');
        final cleared = delegate.setHoveredElement(hovered, null);
